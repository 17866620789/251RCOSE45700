...
names = ["bot","bot","bot","bot","bot","bot","bot","bot","bot","bot","bot","bot","bot","bot",
];
            ...
            if (i != j) {
                ...
                if (!kt) {
                    for (let k = 0; k < mySnake[i].v.length; k += 5) {
                        FOOD[index] = new food(this, this.getSize() / (2 + Math.random() * 2), 
                            mySnake[i].v[k].x + Math.random() * mySnake[i].size / 2, 
                            mySnake[i].v[k].y + Math.random() * mySnake[i].size / 2);
                    ...
                    if (i != 0) {
                        if (j === 0) {
                            playerKills++; 
                            document.getElementById("snake-kill").textContent = playerKills; // 更新显示
                        }
                        mySnake[i] = new snake(
                        ...
                        // 玩家死亡
                        document.getElementById('die-sfx').play();
                        document.getElementById("snake-length").textContent = Math.floor(mySnake[i].score);
                        document.getElementById("snake-kill").textContent = playerKills; // 显示最终击杀数
                        document.getElementById("game-end-scene").style.display = "flex";
                        die = true;
                        return;
                    }
                }
